...
### 1. Clone and build moxygen

Clone [moxygen](https://github.com/facebookexperimental/moxygen) and build it:

```bash
git clone git@github.com:facebookexperimental/moxygen.git
cd moxygen
...
From this project's directory (moq-encoder-player), generate ECDSA certificates and fingerprint:

```bash
./create_self_signed_certs.sh
```

This creates `certs/certificate.pem`, `certs/certificate.key`, and `certs/certificate_fingerprint.hex`.

Note: The certificates are ECDSA with 10-day validity (<15 days), which is required for Chrome to accept them via WebTransport's `serverCertificateHashes`. These same certs are used for both moxygen and the browser client.

### 3. Run moxygen relay server

From the **moxygen** directory:

```bash
MOXYGEN_BUILD_PATH=$(./build/fbcode_builder/getdeps.py show-build-dir moxygen)
```

Then run the relay using the ECDSA certificates from this project (replace `<path-to-moq-encoder-player>` with the absolute path to this project's directory):

```bash
$MOXYGEN_BUILD_PATH/moxygen/relay/moqrelayserver --cert <path-to-moq-encoder-player>/certs/certificate.pem --key <path-to-moq-encoder-player>/certs/certificate.key --endpoint "/moq" --logging=DBG4 --port 4433
```

You should see output like:
```
MoQServer: Registering ALPN handlers: moqt-16, moqt-15, moq-00
```

**Important:**
- You **must** use the ECDSA certificates generated by this project (step 2), not the RSA certificates from moxygen's own `scripts/create-server-certs.sh`. WebTransport's `serverCertificateHashes` only works with ECDSA certificates.
- Make sure **no other instance** of `moqrelayserver` is already running on port 4433. If there is, kill it first (`pkill moqrelayserver`), otherwise the new instance will fail silently and the old one (potentially with wrong certs) will keep serving.
- The certificates expire after 10 days. If you get `CERTIFICATE_VERIFY_FAILED` errors after they worked before, regenerate them (step 2) and restart moxygen.

### 4. Run the encoder-player web server

In a **separate terminal**, from this project's directory:
